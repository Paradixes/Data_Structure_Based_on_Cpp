...
int TMyDate::DaysBetween() //日期间时间差
{
	...
	localtime_s(tp,&t);
	int diff_t = abs(JulianDay(tp->tm_year + 1900, tp->tm_mon + 1, tp->tm_mday) - JulianDay(year, month, day));
	return diff_t;
}
...
{
	int diff_t = abs(JulianDay(year, month, day) - JulianDay(t.year, t.month, t.day));
	return diff_t;
}
